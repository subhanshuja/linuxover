...
# preliminary 
apt -t buster-backports update && apt -t buster-backports dist-upgrade -fy
...
# nvidia
apt install -t buster-backports nvidia-smi nvidia-driver nvidia-cuda-toolkit mesa-utils

#if problem with nouveau and nvidia
Xorg -continue
cp -R /root/xorg.conf.new /etc/X11/xorg.conf

# complete installation for coding
apt install default-jre default-jdk npm yarn

# monitor performance
apt install htop lshw

# designer, office & optional
